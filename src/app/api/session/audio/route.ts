...
import { batchTTS } from "@/lib/session-generator";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const results = await batchTTS(openai, items);

    console.log(`[TIMING] Audio batch (${items.length} items): ${Date.now() - t0}ms`);
    ...
    return Response.json({ error: `Audio generation failed: ${message}` }, { status: 500 });
  }
}
